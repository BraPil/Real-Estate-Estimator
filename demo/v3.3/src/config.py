...
class Settings(BaseSettings):
    ...
    model_path: str = Field(
        default="model/model.pkl", description="Path to the pickled sklearn model"
    )
    features_path: str = Field(
        default="model/model_features.json", description="Path to the model features JSON file"
    )
    demographics_path: str = Field(
        default="data/zipcode_demographics.csv", description="Path to the demographics CSV file"
    )

    # MLflow integration (optional)
    mlflow_tracking_uri: str | None = Field(default=None, description="MLflow tracking server URI")
    mlflow_model_name: str = Field(
        default="real-estate-price-predictor", description="MLflow model registry name"
    )
    mlflow_model_stage: str | None = Field(
        default="Production",
