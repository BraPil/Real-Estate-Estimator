...
def create_model_configs() -> dict[str, dict[str, Any]]:
    ...
    models["KNN (V2.3 Baseline)"] = {
        "model": KNeighborsRegressor(n_neighbors=7, weights="distance", metric="manhattan"),
...
def evaluate_model(
    ...
    cv_mae_scores = cross_val_score(
        pipeline, X_train, y_train, cv=cv_folds, scoring="neg_mean_absolute_error", n_jobs=-1
    )
    cv_r2_scores = cross_val_score(pipeline, X_train, y_train, cv=cv_folds, scoring="r2", n_jobs=-1)
...
def run_comparison(
    ...
    for model_name, config in model_configs.items():
        ...
        # Evaluate
        metrics = evaluate_model(pipeline, X_train, y_train, X_test, y_test, cv_folds=CV_FOLDS)
...
def print_comparison_table(results_df: pd.DataFrame):
    ...
    for _, row in results_df.iterrows():
        ...
        print(
            f"{model_name:<25} {cv_mae:>15} {test_mae:>12} {test_r2:>10} {test_rmse:>12} {time_s:>8}"
        )
...
def save_results(
    ...
    summary = {
        ...
        "knn_baseline": {
            "test_mae": float(
                results_df[results_df["model_name"].str.contains("KNN")].iloc[0]["test_mae"]
            )
        },
...
def generate_recommendation(results_df: pd.DataFrame) -> str:
    ...
    if "KNN" in best_name:
        ...
        if improvement_pct < 5:
            lines.append(
                "⚠️  However, improvement is < 5%. Consider if added complexity is worth it."
            )
...
def main():
    ...
    parser.add_argument(
        "--experiment-name", default="real-estate-v2.4-comparison", help="MLflow experiment name"
    )
    ...
    save_results(results_df, importance_dict, best_pipeline, feature_names, args.experiment_name)
