...
def main():
    ...
    for i in range(min(NUM_EXAMPLES, len(df))):
        ...
        if all(p is not None for p in [price_predict, price_minimal, price_full]):
            results.append({
                ...
                'diff_minimal_vs_full': abs(price_predict - price_minimal) - abs(price_predict - price_full),
            })
            
            print(f"Example {i+1} (ZIP {payload_predict['zipcode']}):")
            print(f"  /predict (gold):    ${price_predict:>12,.2f}")
            print(f"  /predict-minimal:   ${price_minimal:>12,.2f}  (diff: ${abs(price_predict - price_minimal):>10,.2f})")
            print(f"  /predict-full:      ${price_full:>12,.2f}  (diff: ${abs(price_predict - price_full):>10,.2f})")
            
            # Which is closer?
            if abs(price_predict - price_full) < abs(price_predict - price_minimal):
                print(f"  → /predict-full is CLOSER by ${abs(price_predict - price_minimal) - abs(price_predict - price_full):,.2f}")
            else:
                print(f"  → /predict-minimal is CLOSER by ${abs(price_predict - price_full) - abs(price_predict - price_minimal):,.2f}")
            print()
    ...
    if results:
        ...
        if improvement > 0:
            ...
            print(f"          Provides ${improvement:,.2f} ({improvement_pct:.1f}%) better predictions on average.")
        else:
            print(" VERDICT: Hypothesis NOT confirmed. /predict-minimal is equally or more accurate.")
            print(f"          Defaults may be well-calibrated for typical homes.")
        print("=" * 70)
