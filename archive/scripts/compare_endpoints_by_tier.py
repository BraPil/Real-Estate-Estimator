...
def main():
    ...
    for i in range(min(NUM_EXAMPLES, len(df))):
        ...
        price_adaptive_api = make_prediction(
            "predict-adaptive", payload_full
        )  # NEW: actual API adaptive
    ...
    for tier in ["LOW", "HIGH"]:
        ...
        print(
            f"  /predict-minimal wins: {minimal_wins}/{len(tier_df)} ({minimal_wins/len(tier_df)*100:.0f}%)"
        )
        print(
            f"  /predict-full wins:    {full_wins}/{len(tier_df)} ({full_wins/len(tier_df)*100:.0f}%)"
        )
    ...
    if len(low_tier) > 0 and len(high_tier) > 0:
        ...
        print(
            f"  LOW tier: /predict-minimal wins {low_minimal_wins_pct:.0f}% {'✓ CONFIRMED' if low_minimal_wins_pct > 50 else '✗ NOT confirmed'}"
        )
        print(
            f"  HIGH tier: /predict-full wins {high_full_wins_pct:.0f}% {'✓ CONFIRMED' if high_full_wins_pct > 50 else '✗ NOT confirmed'}"
        )

        if low_minimal_wins_pct > 50 and high_full_wins_pct > 50:
            print("\n✓ USER HYPOTHESIS CONFIRMED!")
            print("  Price-tier adaptive strategy is justified.")
