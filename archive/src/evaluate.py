...
def analyze_error_by_price_range(y_true: np.ndarray, y_pred: np.ndarray) -> dict:
...
def evaluate_model(
    ...
    print(
        f"Predicted Range: ${predictions['predicted_min']:,.0f} - ${predictions['predicted_max']:,.0f}"
    )
    ...
    for range_name, stats in error_by_range.items():
        print(
            f"{range_name:15s}: n={stats['count']:4d}, MAE=${stats['mae']:,.0f}, RMSE=${stats['rmse']:,.0f}"
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--model-path", "-m", type=str, default=MODEL_PATH, help="Path to trained model pickle file"
    )
    parser.add_argument(
        "--features-path",
        "-f",
