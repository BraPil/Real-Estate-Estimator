...
def tune_model(
    ...
    # Create pipeline
    pipeline = Pipeline([("scaler", RobustScaler()), ("model", base_models[model_name])])
...
def run_tuning(models_to_tune: list[str] = None, n_iter: int = 20) -> pd.DataFrame:
    ...
    for model_name in models_to_tune:
        try:
            results = tune_model(model_name, X_train, y_train, X_test, y_test, n_iter)
...
def save_results(results_df: pd.DataFrame, best_pipelines: dict, feature_names: list[str]):
...
def main():
    parser = argparse.ArgumentParser(description="V2.4.1: Light tuning for top 3 models")
    parser.add_argument(
        "--n-iter",
        type=int,
        default=20,
    ...
    results_df, best_pipelines, feature_names = run_tuning(
        models_to_tune=args.models, n_iter=args.n_iter
    )
    ...
    print("\nNext steps:")
    print("   1. Copy tuned model to model/model.pkl")
    print("   2. Update model/metrics.json with new performance")
    print("   3. Test API with new model")
    print("   4. Validate on future_unseen_examples.csv")
    print("=" * 70 + "\n")
