...
def kfold_evaluation(X: pd.DataFrame, y: pd.Series, k_folds: int = 5) -> dict:
    """
    ...
    pipeline = Pipeline(
        [("scaler", RobustScaler()), ("model", XGBRegressor(**get_xgboost_params()))]
    )
...
def bootstrap_confidence_intervals(
    ...
    logger.info(
        f"  MAE:  ${results['mae']['mean']:,.0f} [{results['mae']['ci_lower']:,.0f}, {results['mae']['ci_upper']:,.0f}]"
    )
    logger.info(
        f"  R²:   {results['r2']['mean']:.4f} [{results['r2']['ci_lower']:.4f}, {results['r2']['ci_upper']:.4f}]"
    )
    logger.info(
        f"  RMSE: ${results['rmse']['mean']:,.0f} [{results['rmse']['ci_lower']:,.0f}, {results['rmse']['ci_upper']:,.0f}]"
    )
...
def log_transform_experiment(X: pd.DataFrame, y: pd.Series, k_folds: int = 5) -> dict:
    """
    ...
    results = {
        ...
        "comparison": {
            ...
            "mae_improvement_pct": (np.mean(normal_mae) - np.mean(log_mae))
            / np.mean(normal_mae)
            * 100,
    ...
    logger.info("\nCOMPARISON RESULTS:")
    logger.info(
        f"  Normal:        MAE ${results['normal']['mae_mean']:,.0f} +/- ${results['normal']['mae_std']:,.0f}"
    )
    logger.info(
        f"  Log Transform: MAE ${results['log_transform']['mae_mean']:,.0f} +/- ${results['log_transform']['mae_std']:,.0f}"
    )

    if results["comparison"]["mae_improvement_pct"] > 0:
        logger.info(
            f"  --> Log transform is {results['comparison']['mae_improvement_pct']:.1f}% BETTER"
        )
    else:
    ...
    if results["normal"]["high_value_mae_mean"]:
        ...
        logger.info(
            f"    Log Transform: MAE ${results['log_transform']['high_value_mae_mean']:,.0f}"
        )
        if results["comparison"]["high_value_improvement_pct"]:
            if results["comparison"]["high_value_improvement_pct"] > 0:
                logger.info(
                    f"    --> Log transform is {results['comparison']['high_value_improvement_pct']:.1f}% BETTER on high-value homes"
                )
            else:
                logger.info(
                    f"    --> Normal is {-results['comparison']['high_value_improvement_pct']:.1f}% BETTER on high-value homes"
                )
...
def residual_analysis(X: pd.DataFrame, y: pd.Series, test_size: float = 0.2) -> dict:
    """
    ...
    results["heteroscedasticity"] = {
        ...
        "is_heteroscedastic": max(variance_by_quartile.values())
        / min(variance_by_quartile.values())
        > 2,
    }
    ...
    for name, data in results["by_price_range"].items():
        logger.info(
            f"  {name:15}: n={data['count']:4}, MAE=${data['mae']:,.0f}, Bias={data['bias']}"
        )

    logger.info("\nDIAGNOSTICS:")
    logger.info(
        f"  Normality Test: p={results['normality_test']['p_value']:.4f} "
        f"({'Normal' if results['normality_test']['is_normal'] else 'Not Normal'})"
    )
    logger.info(
        f"  Heteroscedasticity: Variance Ratio={results['heteroscedasticity']['variance_ratio']:.2f} "
...
def main():
    ...
    all_results = {
        ...
        "version": "v2.5",
        "data": {"n_samples": len(X), "n_features": len(feature_names)},
    }
    ...
    if not args.skip_bootstrap:
        bootstrap_results = bootstrap_confidence_intervals(
            X, y, n_bootstrap=args.bootstrap_samples, confidence=args.confidence
        )
    ...
    print(
        f"  Heteroscedastic: {'Yes' if residual_results['heteroscedasticity']['is_heteroscedastic'] else 'No'}"
    )
    ...
    if log_results["comparison"]["mae_improvement_pct"] > 2:
        ...
        recommendations.append(
            "Log transform shows minimal difference - keep current model for simplicity"
        )
    ...
    if abs(residual_results["basic_stats"]["mean_residual"]) > 5000:
        ...
        recommendations.append(
            f"Model has systematic bias ({bias_dir}predicting by ${abs(residual_results['basic_stats']['mean_residual']):,.0f})"
        )
