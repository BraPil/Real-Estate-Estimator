...
def run_grid_search(X_train, y_train, cv_folds=5):
    ...
    # Create pipeline
    pipeline = Pipeline([("scaler", RobustScaler()), ("knn", KNeighborsRegressor())])
    ...
    if MLFLOW_AVAILABLE:
        try:
            ...
            with mlflow.start_run(
                run_name=f"v2.3-tuning-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            ):
                # Log parameters
...
def main():
    ...
    parser.add_argument(
        "--experiment-name", default="real-estate-v2.3-tuning", help="MLflow experiment name"
    )
    ...
    # Run grid search
    grid_search, results_df = run_grid_search(X_train, y_train, cv_folds=args.cv_folds)

    # Evaluate on test set
    test_metrics = evaluate_best_model(grid_search, X_test, y_test)

    # Save results
    save_results(grid_search, results_df, test_metrics, feature_names, args.experiment_name)
